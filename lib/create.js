...
class Creator {
    ...
    async changePackage() {
        await fs.readFile(`${cwd}/${this.projectName}/package.json`, (err, data) => {
            if (err) throw err;
            
            let _data = JSON.parse(data.toString());
            console.log(123123,data.toString())
            _data.name = this.projectName;
            _data.version = '1.0.0';
            let str = JSON.stringify(_data, null, 4);
            fs.writeFile(`${cwd}/${this.projectName}/package.json`, str, function (err) {
                if (err) throw err;
            })
        });
    };
    ...
    async getCollectRepo() {
        ...
        loading.succeed();
        const collectTemplateNameList = list.filter(item => item.topics.includes('components')).map(item => item.name);
    ...
    async downloadTemplate(choiceTemplateName) {
        ...
        const templateUrl = `dawn-ylcode/${choiceTemplateName}`;
        const loading = ora('正在拉取模版...'+cwd+'123'+ this.projectName+'123123'+ templateUrl);
        loading.start();
